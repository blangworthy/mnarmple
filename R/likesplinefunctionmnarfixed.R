...
likesplinefunctionmnarfixed <- function(dat,params,mnarparams,timevar,xvarsmiss,xvarscox,splinevars,ncause){
  ...
  ###parameters for hazard ratio
  for(i in 2:ncause){
    assign(paste0("e",i),params[grep(paste0("e",i),names(params))])
  }
  b <- do.call(cbind,mget(paste0("b",1:ncause)))
  e <- do.call(cbind,mget(paste0("e",2:ncause)))
  ...
  basrat <- cbind(tcrossprod(as.matrix(dat[,..splinevars]),t(e)))
